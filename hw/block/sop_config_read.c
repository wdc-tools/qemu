...
static void config_space_write(PQIState *,  FILERead*, uint8_t, uint16_t);

/*********************************************************************
    Function     :    pqi_read_config_file
    Description  :    Function for reading config files
    Return Type  :    int (0 : Success , 1 : Error)
    Arguments    :    FILE *    : Config File Pointer
                      PQIState * : Pointer to PQI device state
                      uint8_t : Flag for type of config space
                              : 0 = PCI Config Space
                              : 1 = PQI Config Space
*********************************************************************/
int pqi_read_config_file(FILE *config_file , PQIState *n, uint8_t flag)
{
    ...
    data.cfg_name = g_malloc(MAX_CHAR_PER_LINE * sizeof(char));

    /* Read the configuration file line by line */

    if (fgets(var_line, MAX_CHAR_PER_LINE, config_file) == NULL) {
        SOP_LOG_NORM("Config File cannot be read");
        return 1;
    }
    do {
        ...
        if (sor_flag == 1) {
            do {
                ...
                if (fgets(var_line, MAX_CHAR_PER_LINE, config_file) == NULL) {
                    SOP_LOG_NORM("Wrong Format between <REG> and </REG>");
                    return 1;
                }
                update_var((char *)var_line, &data, &eor_flag, &sor_flag);
            } while (eor_flag != 1);
            ...
            if (((data.offset + offset + data.len) > pci_config_size(&n->dev))
                && (flag == PCI_SPACE)) {
                SOP_LOG_ERR("Invlaid Offsets present in the Config file for \
                    PCI address space ");
            } else if (((data.offset + offset + data.len) > 0xFFF) && (flag == PQI_SPACE)) {
                SOP_LOG_ERR("Invlaid Offsets present in the Config file for \
                    SOP address space ");
            } else {
                /* PCI / SOP space writes */
                config_space_write(n, &data, flag, offset);
            }

        }
skip:
        /* Reseting the data structure */
        data.len = 0;
        data.offset = 0;
        data.val = 0;
        data.ro_mask = 0;
...
/*********************************************************************
    ...
    Return Type  :    void
    Arguments    :    PQIState * : Pointer to SOP device state
...
*********************************************************************/
static void config_space_write(PQIState *n, FILERead* data, uint8_t flag, uint16_t offset)
{
  // Pointer to config space
  uint8_t *conf = NULL;
  // Pointers for masks
  uint8_t *wmask = NULL, *used = NULL, *cmask = NULL, *w1cmask = NULL, *w1smask = NULL;
  ...
  if(PCI_SPACE == flag)
  {
    ...
    for (index = 0; index < data->len; data->val >>= 8, data->rw_mask >>= 8, data->rwc_mask >>= 8, index++)
    {
  ...
  else if(PQI_SPACE == flag)
  {
    conf = n->cntrl_reg;
    wmask = n->rw_mask; // R/W set if 1
    w1cmask = n->rwc_mask; // W1C cleared if 1
    w1smask = n->rws_mask; // W1S set if 1
    used = n->used_mask; // Used/Reserved
    // Write to config Space
    for(index = 0; index < data->len; data->val >>= 8, data->rws_mask >>= 8, data->rw_mask >>= 8, data->rwc_mask >>= 8, index++)
    {
      conf[data->offset + offset + index] = data->val;
      wmask[data->offset + offset + index] = data->rw_mask;
